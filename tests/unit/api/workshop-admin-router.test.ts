...
describe("workshop.togglePublish", () => {
  it("toggles publish status", async () => {
    vi.mocked(prisma.workshop.update).mockResolvedValue({ id: "w1", isPublished: false } as any);
    const result = await caller.workshop.togglePublish({ id: "w1", isPublished: false });
...
describe("workshop.delete", () => {
  it("deletes a workshop", async () => {
    ...
    expect(prisma.workshop.delete).toHaveBeenCalledWith({ where: { id: "w1" } });
  });
});
...
describe("workshop.createWorkshop", () => {
  it("creates a workshop with nested price tiers and variants", async () => {
    const input = {
      ...
      video: null,
      priceTiers: [
        { groupSize: "8-10", minParticipants: 8, maxParticipants: 10, priceExclBtw: 70, priceInclBtw: 85, sortOrder: 0 },
      ],
      variants: [
        {
          name: "Basic",
          ...
          sortOrder: 0,
          priceTiers: [
            { groupSize: "8-10", minParticipants: 8, maxParticipants: 10, priceExclBtw: 50, priceInclBtw: 60, sortOrder: 0 },
          ],
        },
      ],
    };

    vi.mocked(prisma.workshop.create).mockResolvedValue({ id: "w-new", ...input } as any);
...
describe("workshop.updateWorkshop", () => {
  it("deletes old and recreates price tiers and variants", async () => {
    const input = {
      ...
      sortOrder: 1,
      priceTiers: [
        { groupSize: "10-15", minParticipants: 10, maxParticipants: 15, priceExclBtw: 60, priceInclBtw: 73, sortOrder: 0 },
      ],
      variants: [],
    };

    vi.mocked(prisma.priceTier.deleteMany).mockResolvedValue({ count: 1 } as any);
    vi.mocked(prisma.workshopVariant.deleteMany).mockResolvedValue({ count: 0 } as any);
    vi.mocked(prisma.workshop.update).mockResolvedValue({ id: "w1", ...input } as any);
...
describe("workshop.create with TBD dates", () => {
  ...
  it("stores customCity when location=other", async () => {
    vi.mocked(prisma.workshopConfig.create).mockResolvedValue({ id: "c1" } as any);
    await caller.workshop.create({
      type: "particulier",
      participantCount: 10,
      workshops: ["kookworkshop"],

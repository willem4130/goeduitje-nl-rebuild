...
test.describe("Feedback Form", () => {
  ...
  test("feedback page loads with form", async ({ page }) => {
    await expect(page.getByText("Geef ons feedback")).toBeVisible({ timeout: 10000 });
  ...
  test("star rating is interactive", async ({ page }) => {
    // Click the 4th star
    const stars = page.locator("button").filter({ has: page.locator("svg.h-10") });
  ...
  test("whatWasBest and whatToImprove textareas are present", async ({ page }) => {
